...
export interface TailscaleStatusPeer {
  ...
  stableID?: string;
  /** rc.2 的完整原生 peer 字段；顶层保留既有 lean 形态以稳定出口选择等高频消费方。 */
  details?: {
    dnsName: string;
    os: string;
    tailscaleIPs: string[];
...
export interface TailscaleUserGroupStatus {
  userID: number;
  loginName: string;
  displayName: string;
...
export interface TailscaleStatusDetails {
  stateText: string;
  networkName: string;
  magicDNSSuffix: string;
  keyAuth: boolean;
  self?: TailscaleStatusPeer;
  userGroups: TailscaleUserGroupStatus[];
  exitNode?: TailscaleStatusPeer;
}
...
export interface TailscaleStatusEvent {
  ...
  peers: TailscaleStatusPeer[];
  /** sing-box 1.14.0-rc.2 的完整原生状态投影。旧后端/测试载荷可缺。 */
