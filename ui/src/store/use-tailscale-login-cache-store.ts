 ...
 *    spawn 瞬态核探针读真实态——为罕见事件付高频代价，过度主动。缓存上次已知态即可秒显；
 *    代理开时由 api STATUS 流自动校正（免费、实时，key 失效在此暴露）。
...
export const useTailscaleLoginCacheStore = create<TailscaleLoginCacheState>(
  (set, get) => ({
    ...
    setCached: (serverId, loggedIn) => {
      ...
      const next = {
        ...get().cache,
        [serverId]: { loggedIn, cachedAt: Date.now() },
      };
      persist(next);
      set({ cache: next });
    },
