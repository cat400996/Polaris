/**
 * sing-box 版本解析（上游 移植对照件）。
 *
 * ⚠️ **本文件运行期零消费者，改这里不会有任何效果。** 唯一 import 者是同目录
 * `core-build.ts`，而后者的运行期函数同样零消费者（全仓只有 `ipc/api-client.ts`
 * 从它 `import type { CoreBuildKind }` 取类型）。版本比较 / 构建来源判定 / reseed
 * 决策的**单一权威在 Rust**：`crates/updater/src/version.rs` +
 * `crates/updater/src/core_build.rs`（金样对拍在 `crates/updater/tests/core_build_golden.rs`）。
 * 两边行为逐字对齐是移植纪律，但生效的只有 Rust 那份 —— 要改语义去改 Rust，
 * 顺带同步这里；只改这里等于没改。
 *
