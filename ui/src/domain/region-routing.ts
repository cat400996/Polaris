...
/**
 * 是否处于「反向（回国）」语义：所在地区流量走代理、其余直连——与默认语义相反。
 *
 * 真机 2026-07-20：`reverse` 唯一 UI 入口是规则页地区卡右下角的小胶囊，主页/状态栏/侧栏/托盘零呈现，
 * 唯一提示是切换瞬间的一次性 toast。用户误触后主页仍只显示「智能」，不知分流语义已反转。本谓词供主页
 * 持久指示器判据（见 screens/home/ReverseRoutingBadge.tsx）。
 *
 * `enabled` 是前置条件：关掉地区分流后 `reverse` 是死数据，此时提示即噪音。
 */
export function isReverseRegionRouting(
  config: { regionRouting?: RegionRoutingConfig } | null | undefined,
): boolean {
  if (!config) return false;
  const rr = effectiveRegionRouting(config);
  return rr.enabled && rr.reverse;
}
