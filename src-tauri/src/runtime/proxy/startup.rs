...
use polaris_core_supervisor::{
    decide_peel, run_config_check, wait_for_core_ready, CoreReadyDeps, CoreReadyOutcome,
    KernelRejection, PeelStep, PortAllocator, PortExclusions, RejectedArray, SingBoxSpawner,
    SpawnRequest, TokioSpawner, WaitForCoreReadyOptions, INVALID_REASON_KERNEL_REJECTED,
    PEEL_TIME_BUDGET,
};
...
pub(super) const CORE_READY_TIMEOUT_MS: u64 = 12_000;
/// 就绪轮询间隔。
///
...
impl ProxyRuntime {
    ...
    pub(super) async fn start_inner(
        ...
        let (
            ...
            let (api_port, update_in_port, probe_proxy_port, pool_ports) =
    ...
    pub(super) async fn wait_ready(&self, api_port: u16, my_gen: u64) -> CoreReadyOutcome {
    ...
    /// 但不阻断代理本身启动。返回 `(api, update_in, probe_proxy, pool_ports)`。
    fn resolve_start_ports(
        &self,
        user_config: &UserConfig,
        control_port: u16,
    ) -> (u16, u16, Option<u16>, Vec<u16>) {
        ...
        let mut probe_ports = PortAllocator::new(TokioPortProvider)
        ...
        (api_port, update_in_port, probe_proxy_port, pool_ports)
    }
